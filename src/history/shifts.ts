...
import { ensureVersion, kvGet, kvSet, kvDel } from './storage';
...
const SHIFT_KEY = (dateISO: string, shift: ShiftKind) =>
  `history:shift:${dateISO}:${shift}`;
const HUDDLE_KEY = (dateISO: string, shift: ShiftKind) =>
  `history:huddle:${dateISO}:${shift}`;
